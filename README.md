...
* 注意ECS配置段的Password是操作系统root账户的密码，而ssr_server配置段的password是SSR服务的密码，不要混淆；建议设定一个固定的系操作统密码，方便SSH登录，比如出现这些情况：1) 敏感时期，GFW有时会比较勤快的封端口但不封IP，这种情况登上去换个端口启动ssr服务即可；2) 日常可以用服务器做其它测试，反正它很闲。
* 如果使用安卓版ssr客户端，建议ssr_server配置段的端口号port设小一点，比如88，因为安卓版的端口号不能随意填写，而是个列表控件……
